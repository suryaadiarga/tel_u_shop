...
/**
 * Comprehensive API Testing Script with JSON Responses
...
function printTest($title, $response, $success = true)
{
    ...
    echo "Response JSON:\n" . json_encode($response['decoded'], JSON_PRETTY_PRINT) . "\n";
    echo str_repeat("-", 80) . "\n";
}

function printError($title, $response)
{
    global $colors;
    echo $colors['red'] . "✗ " . $title . $colors['reset'] . "\n";
    echo "HTTP Code: " . $response['http_code'] . "\n";
    echo "Response JSON:\n" . json_encode($response['decoded'], JSON_PRETTY_PRINT) . "\n";
    echo str_repeat("-", 80) . "\n";
}
...
echo "\n" . str_repeat("=", 80) . "\n";
echo "📋 COMPREHENSIVE API TEST SUITE WITH JSON RESPONSES\n";
echo str_repeat("=", 80) . "\n\n";
...
if ($registerRes['decoded']['status'] === 'success') {
    $customerToken = $registerRes['decoded']['data']['access_token'];
    $customerId = $registerRes['decoded']['data']['user']['id'];
    printTest("Customer Registration", $registerRes);
} else {
...
if ($registerRes['decoded']['status'] === 'success') {
    $merchantToken = $registerRes['decoded']['data']['access_token'];
    $merchantId = $registerRes['decoded']['data']['user']['id'];
    printTest("Merchant Registration", $registerRes);
} else {
...
if ($loginRes['decoded']['status'] === 'success') {
    $customerToken = $loginRes['decoded']['data']['access_token']; // Use login token
...
if ($meRes['decoded']['status'] === 'success') {
...
if ($productRes['decoded']['status'] === 'success') {
    $productId = $productRes['decoded']['data']['id'];
    printTest("Create Product as Merchant", $productRes);
} else {
...
if ($productsRes['decoded']['status'] === 'success' && count($productsRes['decoded']['data']) > 0) {
...
if ($balanceRes['decoded']['status'] === 'success') {
    printTest("Check Wallet Balance", $balanceRes);
} else {
...
if ($topupRes['decoded']['status'] === 'success') {
    printTest("Top Up Wallet", $topupRes);
} else {
...
if ($transRes['decoded']['status'] === 'success') {
    printTest("Get Wallet Transactions", $transRes);
} else {
...
if ($addCartRes['decoded']['status'] === 'success') {
    $cartItemId = $addCartRes['decoded']['data']['id'];
    printTest("Add Product to Cart", $addCartRes);
} else {
...
if ($cartRes['decoded']['status'] === 'success') {
    printTest("View Cart", $cartRes);
} else {
...
if ($updateRes['decoded']['status'] === 'success') {
    printTest("Update Cart Item Quantity", $updateRes);
} else {
...
if ($checkoutRes['decoded']['status'] === 'success') {
    $orderId = $checkoutRes['decoded']['data']['order_id'];
    printTest("Checkout Order", $checkoutRes);
} else {
...
if ($ordersRes['decoded']['status'] === 'success') {
    printTest("Get Customer Orders/Activities", $ordersRes);
} else {
...
if ($merchantOrdersRes['decoded']['status'] === 'success') {
...
if ($updateStatusRes['decoded']['status'] === 'success') {
    printTest("Update Order Status (Merchant)", $updateStatusRes);
} else {
...
if ($adminRes['decoded']['status'] === 'success') {
    $adminToken = $adminRes['decoded']['data']['access_token'];
    printTest("Create Admin Account", $adminRes);

    // Get All Users
    $usersRes = apiCall('GET', '/admin/users', [], $adminToken);
    if ($usersRes['decoded']['status'] === 'success') {
        printTest("Admin Get All Users", $usersRes);
    } else {
    ...
    if ($allOrdersRes['decoded']['status'] === 'success') {
        printTest("Admin Get All Orders", $allOrdersRes);
    } else {
    ...
    if ($updateRoleRes['decoded']['status'] === 'success') {
        printTest("Admin Update User Role", $updateRoleRes);
    } else {
...
if ($logoutRes['decoded']['status'] === 'success') {
