...
const banners = [
    { id: 1, title: 'Flash Sale 11.11', subtitle: 'Diskon sampai 70%', bg: 'var(--primary)' },
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="relative w-full">
                ...
                <div className="relative flex-1">
                    <input
                        ...
                        className="w-full rounded-xl bg-[var(--primary)] bg-opacity-90 text-[var(--primary-foreground)] placeholder-[var(--primary-foreground)] pl-10 pr-12 py-2.5 focus:outline-none focus:ring-2 focus:ring-white/30"
                    />
                    <svg
                        className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--primary-foreground)]/90"
                    ...
                    {/* Move notification button inside the relative container so it sits above the input */}
                    <button className="absolute top-1/2 -translate-y-1/2 right-3 z-20 rounded-full border-2 border-[var(--primary)] p-2 hover:bg-[var(--hover-bg)]/5 transition bg-white">
                        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" className="text-[var(--primary)]" stroke="currentColor" strokeWidth="2">
                            <path d="M18 8a6 6 0 10-12 0c0 7-3 9-3 9h18s-3-2-3-9" />
                            <path d="M13.73 21a2 2 0 01-3.46 0" />
                        </svg>
                        <span className="absolute -top-1 -right-1 h-4 w-4 rounded-full bg-[var(--primary)] text-[var(--primary-foreground)] text-[10px] flex items-center justify-center">3</span>
                    </button>
                </div>
                </div>
            </div>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3">
                <div className="rounded-2xl bg-white shadow p-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-xl bg-[var(--primary)] bg-opacity-10 flex items-center justify-center">
                            <span className="text-[var(--primary)] text-xl" aria-hidden="true">💵</span>
                        </div>
                        <div>
                            <p className="text-sm text-gray-500">Saldo</p>
                            <p className="text-lg font-semibold text-[var(--primary)]">Rp 45.000</p>
                        </div>
                    </div>
                    <Link href="/topup" className="text-sm font-medium text-[var(--primary)] hover:underline">Top Up</Link>
                </div>
            ...
            <div className="mt-5">
                <div className="overflow-x-auto no-scrollbar">
                    <div className="flex gap-3 w-max">
                        {banners.map(b => (
                                <div key={b.id} className="min-w-[280px] md:min-w-[360px] rounded-2xl text-white p-5 shadow" style={{ background: b.bg }}>
            ...
            <section className="mt-6">
                <div className="flex items-center justify-between mb-2">
                    <h3 className="font-semibold">Flash Sale <span className="ml-2 text-xs rounded bg-[var(--primary)]/10 text-[var(--primary)] px-2 py-0.5">Berakhir 02:15:37</span></h3>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {flashSale.map(item => (
                        <div key={item.id} className="rounded-2xl bg-white shadow p-3 hover:shadow-md transition">
                            ...
                            <button className="mt-2 w-full rounded-lg bg-[var(--primary)] text-[var(--primary-foreground)] text-sm py-2 hover:bg-[var(--primary)]/80">Tambah</button>
                        </div>
                    ))}
                </div>
            </section>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                    {recommended.map(p => (
                        <div key={p.id} className="rounded-2xl bg-white shadow p-3 hover:shadow-md transition">
                            ...
                            <p className="mt-1 font-semibold text-[var(--primary)]">Rp {p.price.toLocaleString('id-ID')}</p>
                            <button className="mt-2 w-full rounded-lg border border-[var(--primary)] text-[var(--primary)] text-sm py-2 hover:bg-[var(--primary)]/5">Tambah</button>
                        </div>
                    ))}
                </div>
            </section>
